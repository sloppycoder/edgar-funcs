...
def send_cloud_run_request(url: str, payload: dict):
    ...
    """
    auth_token = _google_cloud_id_token(audience=url)
    if not auth_token:
...
def create_publisher():
    credentials = _google_cloud_credentials(
        scopes=["https://www.googleapis.com/auth/pubsub"]
    )
    if credentials:
        return pubsub_v1.PublisherClient(credentials=credentials)
    else:
        # GOOGLE_APPLICATION_CREDENTIALS not specified, use default credentials.
        # Under Cloud Run it just uses the service account of the Cloud Run service
...
def _google_cloud_credentials(scopes: list[str]) -> service_account.Credentials | None:
